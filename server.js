...
function broadcastLobby(lobby,event,data={}){
  lobby.io.to(lobby.code).emit(event,{...data,code:lobby.code,players:listPlayers(lobby)});
}
...
io.on("connection",socket=>{
  ...
  socket.on("gameMessage",msg=>{
    ...
    if(!lobby || !p) return;
    if(msg.type==="start"){
      if(!p.host || !validSettings(msg.settings) || lobby.gameStarted) return;
      lobby.settings=msg.settings; lobby.total=totalQuestions(msg.settings.gameMode); lobby.gameStarted=true; lobby.questionIndex=0;
      for(const x of lobby.players.values()){x.score=0;x.streak=0;x.bestStreak=0;}
      broadcastLobby(lobby,"gameStart",{settings:lobby.settings,totalQuestions:lobby.total===Infinity?null:lobby.total});
      setTimeout(()=>startQuestion(lobby),500);
